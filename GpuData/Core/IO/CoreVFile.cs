	...
	public class CoreVFile : VFile
	{
		...
		public override RC Write(byte[] buffer, int amount, long offset)
		{
			...
			if (rc == 0 || amount > (int)wrote)
			{
				...
				return (LastErrorID == ERROR_HANDLE_DISK_FULL || LastErrorID == ERROR_DISK_FULL ? RC.FULL : winLogError(RC.IOERR_WRITE, "winWrite", Path));
			}
			return RC.OK;
		}
		...
		public virtual RC CheckReservedLock(ref int outRC)
		{
			int rc;
		...
		private int seekWinFile(long iOffset)
		{
			...
			catch (Exception e) { LastErrorID = (uint)Marshal.GetLastWin32Error(); WIN._Error(RC.IOERR_SEEK, "seekWinFile", Path); return 1; }
			return 0;
		}
