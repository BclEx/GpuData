...
#define PAGERTRACE(X) if (OSTrace) { printf X; }
#else
#define PAGERTRACE(X)
#endif
...
#define DEFAULT_JOURNAL_SIZE_LIMIT -1
	...
	int sqlite3JournalOpen(VFileSystem *, const char *, VFile *, int, int);
	int sqlite3JournalSize(VFileSystem *);
	int sqlite3JournalCreate(VFile *);
	int sqlite3JournalExists(VFile *p);
#else
#define sqlite3JournalSize(vfs) ((vfs)->SizeOsFile)
#define sqlite3JournalExists(p) true
#endif
...
#define RollbackWal(x) RC::OK
#define WalFrames(v,w,x,y) RC::OK
#define OpenWalIfPresent(z) RC::OK
#define BeginReadTransaction(z) RC::OK
#endif
...
#pragma region Name1
		...
		static RC pagerUnlockDb(Pager *pager, VFile::LOCK lock)
		{
		...
		static RC pagerLockDb(Pager *pager, VFile::LOCK lock)
		{
...
#pragma region Journal1
		...
		static RC readJournalHdr(Pager *pager, bool isHot, int64 journalSize, uint32 *recordsOut, uint32 *dbSizeOut)
		{
			_assert(pager->JournalFile->Opened);
...
#pragma region Name2
		...
		static void releaseAllSavepoints(Pager *pager)
		{
			...
			if (!pager->ExclusiveMode || VFile::IsMemoryVFile(pager->SubJournalFile))
		...
		static void pager_unlock(Pager *pager)
		{
			...
			else if (!pager->ExclusiveMode)
			{
				...
				if ((dc & VFile::IOCAP::UNDELETABLE_WHEN_OPEN) == 0 || (pager->JournalMode & 5) != 1)
...
#pragma region Transaction1
		...
		static RC pager_truncate(Pager *pager, Pid pages);

		static RC pager_end_transaction(Pager *pager, bool hasMaster, bool commit)
		{
			...
			if (pager->JournalFile->Opened)
			{
				...
				if (VFile::IsMemoryVFile(pager->JournalFile))
				{
		...
		static RC pager_playback_one_page(Pager *pager, int64 *offset, Bitvec *done, bool isMainJournal, bool isSavepoint)
		{
			...
			*offset += pager->PageSize + 4 + (isMainJournal ? 4 : 0); //TODO: CHECK THIS
			...
			if (isMainJournal)
			{
				...
				if (rc) return rc;
				if (!isSavepoint && pager_cksum(pager, (uint8*)data) != checksum)
					return RC::DONE;
			}
			...
			if (id == 1 && pager->ReserveBytes != ((uint8 *)data)[20])
			{
				pager->ReserveBytes = ((uint8 *)data)[20];
...
#pragma region Transaction2
		...
		static void setSectorSize(Pager *pager)
		{
			...
			if (pager->TempFile || (pager->File->get_DeviceCharacteristics() & VFile::IOCAP::POWERSAFE_OVERWRITE) != 0)
		...
		static RC pager_playback(Pager *pager, bool isHot)
		{
			...
			// for pageSize.
			VFileSystem *vfs = pager->Vfs;
			char *master = pager->TmpSpace; // Name of master journal file if any
...
end_playback:
...
#ifdef _DEBUG
			pager->File->FileControl(VFile::FCNTL::DB_UNCHANGED, 0);
#endif
			...
			if (rc == RC::OK)
			{
				master = pager->TmpSpace;
				rc = readMasterJournal(pager->JournalFile, master, vfs->MaxPathname + 1);
				ASSERTCOVERAGE(rc != RC::OK);
			}
			if (rc == RC::OK && (pager->State >= PAGER::WRITER_DBMOD || pager->State == PAGER::OPEN))
		...
		static RC readDbPage(PgHdr *pg)
		{
			Pager *pager = pg->Pager; // Pager object associated with page pPg

			_assert(pager->State >= PAGER::READER && !pager->MemoryDB);
			_assert(pager->File->Opened);
			...
			if (UseWal(pager)) // Try to pull the page from the write-ahead log.
				rc = sqlite3WalRead(pager->Wal, id, &isInWal, pageSize, pg->Data);
			if (rc == RC::OK && !isInWal)
			{
				int64 offset = (id - 1) * (int64)pager->PageSize;
			...
			CODEC1(pager, pg->Data, id, 3, rc = RC::NOMEM);
		...
		static void pager_write_changecounter(PgHdr *pg)
		{
			...
			ConvertEx::Put4(((uint8 *)pg->Data) + 96, SysEx::VERSION_NUMBER);
		}

#ifndef OMIT_WAL
		static RC pagerUndoCallback(void *ctx, Pid id)
		{
			RC rc = RC::OK;
			Pager *pager = (Pager *)ctx;
			PgHdr *pg = sqlite3PagerLookup(pager, id);
			if (pg)
			{
		...
		static int pagerRollbackWal(Pager *pager)
		{
			...
			RC rc = sqlite3WalUndo(pager->Wal, pagerUndoCallback, (void *)pager);
		...
		static int pagerWalFrames(Pager *pager, PgHdr *list, Pid nTruncate, int isCommit)
		{
			_assert(pager->Wal);
			...
			RC rc = sqlite3WalFrames(pager->Wal, pager->PageSize, list, nTruncate, isCommit, pager->WalSyncFlags);
		...
		{
			_assert(UseWal(pager));
			_assert(pager->State == PAGER::OPEN || pager->State == PAGER::READER);
			...
			sqlite3WalEndReadTransaction(pager->Wal);

			int changed = 0; // True if cache must be reset
			RC rc = sqlite3WalBeginReadTransaction(pager->Wal, &changed);
			if (rc != RC::OK || changed)
				pager_reset(pager);
		...
		static RC pagerPagecount(Pager *pager, Pid *pagesOut)
		{
			...
			Pid pages = sqlite3WalDbsize(pager->Wal);
		...
		static int pagerOpenWalIfPresent(Pager *pager)
		{
			...
			if (!pager->TempFile)
			{
				...
				if (pages == 0)
				{
					rc = pager->Vfs->Delete(pager->zWal, false);
					if (rc == RC::IOERR_DELETE_NOENT) rc = RC::OK;
					isWal = 0;
				}
				else
					rc = pager->Vfs->Access(pager->zWal, VFileSystem::ACCESS::EXISTS, &isWal);
				if (rc == RC::OK)
				{
					if (isWal)
					{
						ASSERTCOVERAGE(pager->PCache->get_Pages() == 0);
						rc = sqlite3PagerOpenWal(pager, 0);
					}
		...
		static RC pagerPlaybackSavepoint(Pager *pager, PagerSavepoint *savepoint)
		{
			_assert(pager->State != PAGER::ERROR);
			...
			if (savepoint && !UseWal(pager))
			{
				...
				while (rc == RC::OK && pager->JournalOffset < hdrOffset)
			...
			while (rc == RC::OK && pager->JournalOffset < sizeJournal)
			{
				...
				for (uint32 ii = 0U; rc == RC::OK && ii < records && pager->JournalOffset < sizeJournal; ii++)
					rc = pager_playback_one_page(pager, &pager->JournalOffset, done, true, true);
				_assert(rc != RC::DONE);
			}
			_assert(rc != RC::OK || pager->JournalOffset >= sizeJournal);
			...
			if (savepoint)
			{
				...
				if (UseWal(pager))
					rc = sqlite3WalSavepointUndo(pager->Wal, savepoint->WalData);
				for (uint32 ii = savepoint->SubRecords; rc == RC::OK && ii < pager->SubRecords; ii++)
				{
...
#pragma region Name3
		...
		void sqlite3PagerSetCachesize(Pager *pager, int maxPage)
		{
			pager->PCache->SetCachesize(maxPage);
		}
		...
		void sqlite3PagerSetSafetyLevel(Pager *pager, int level, bool fullFsync, bool checkpointFullFsync)
		{
			_assert(level >= 1 && level <= 3);
			pager->NoSync =  (level == 1 || pager->TempFile);
			pager->FullSync = (level == 3 && !pager->TempFile);
			if (pager->NoSync)
			{
				pager->SyncFlags = 0;
				pager->CheckpointSyncFlags = 0;
			}
			else if (fullFsync)
			{
				pager->SyncFlags = VFile::SYNC::FULL;
				pager->CheckpointSyncFlags = VFile::SYNC::FULL;
			}
			...
			pager->WalSyncFlags = pager->SyncFlags;
			if (pager->FullSync)
				pager->WalSyncFlags |= VFile::SYNC::WAL_TRANSACTIONS;
		}
#endif
...
#endif
			vfsFlags |=  VFileSystem::OPEN::OREADWRITE | VFileSystem::OPEN::CREATE | VFileSystem::OPEN::EXCLUSIVE | VFileSystem::OPEN::DELETEONCLOSE;
			RC rc = pager->Vfs->Open(nullptr, file, vfsFlags, nullptr);
			_assert(rc != RC::OK || file->Opened);
			return rc;
		}

		void sqlite3PagerSetBusyhandler(Pager *pager, int (*busyHandler)(void *), void *busyHandlerArg)
		{
			pager->BusyHandler = busyHandler;
			pager->BusyHandlerArg = busyHandlerArg;

			if (pager->File->Opened)
			{
				void **ap = (void **)&pager->BusyHandler;
				_assert(((int(*)(void *))(ap[0])) == busyHandler);
				_assert(ap[1] == busyHandlerArg);
				pager->File->FileControl(VFile::FCNTL::BUSYHANDLER, (void *)ap);
			}
		}

		RC sqlite3PagerSetPagesize(Pager *pager, uint32 *pageSizeRef, int reserveBytes)
		{
			...
			RC rc = RC::OK;
			if ((!pager->MemoryDB || pager->DBSize == 0) &&
				pager->PCache->get_Refs() == 0 &&
				pageSize && pageSize != (uint32)pager->PageSize)
			{
				int64 bytes = 0;
				if (pager->State > PAGER::OPEN && pager->File->Opened)
					rc = pager->File->get_FileSize(&bytes);
				uint8 *tempSpace = nullptr; // New temp space
				if (rc == RC::OK)
				{
				...
				{
					pager_reset(pager);
					pager->DBSize = (Pid)((bytes + pageSize - 1) / pageSize);
					pager->PageSize = (int)pageSize;
					sqlite3PageFree(pager->TmpSpace);
					pager->TmpSpace = tempSpace;
					pager->PCache->SetPageSize(pageSize);
				}
			}
			*pageSizeRef = (uint)pager->PageSize;
			if (rc == RC::OK)
			{
				if (reserveBytes < 0) reserveBytes = pager->ReserveBytes;
				_assert(reserveBytes >= 0 && reserveBytes < 1000);
				pager->ReserveBytes = (int16)reserveBytes;
				pagerReportSize(pager);
			}
			return rc;
		}
...
#define enable_simulated_io_errors()
		...
		RC sqlite3PagerReadFileheader(Pager *pager, int n, unsigned char *dest)
		{
			_memset(dest, 0, n);
			_assert(pager->File->Opened || pager->TempFile);
		...
		void sqlite3PagerPagecount(Pager *pager, Pid *pagesOut)
		{
			_assert(pager->State >= PAGER::READER);
			_assert(pager->State != PAGER::WRITER_FINISHED);
			*pagesOut = pager->DBSize;
		}

		static RC pager_wait_on_lock(Pager *pager, VFile::LOCK locktype)
		{
			...
			_assert((pager->Lock >= locktype) ||
				(pager->Lock == VFile::LOCK::NO && locktype == VFile::LOCK::SHARED) ||
				(pager->Lock == VFile::LOCK::RESERVED && locktype == VFile::LOCK::EXCLUSIVE));

			RC rc;
			do
			{
				rc = pagerLockDb(pager, locktype);
			} while(rc == RC::BUSY && pager->BusyHandler(pager->BusyHandlerArg));
			return rc;
		}

#if defined(_DEBUG)
		static void assertTruncateConstraintCb(PgHdr *pg)
		{
			_assert(pg->Flags & PgHdr::PGHDR::DIRTY);
			_assert(!subjRequiresPage(pg) || pg->ID <= pg->Pager->DBSize);
		}

		static void assertTruncateConstraint(Pager *pager)
		{
...
#define assertTruncateConstraint(pager)
		...
		void sqlite3PagerTruncateImage(Pager *pager, Pid pages)
		{
			_assert(pager->DBSize >= pages);
		...
		static RC pagerSyncHotJournal(Pager *pager)
		{
			RC rc = RC::OK;
			if (!pager->NoSync)
				rc = pager->JournalFile->Sync(VFile::SYNC::NORMAL);
			if (rc == RC::OK)
				rc = pager->JournalFile->get_FileSize(&pager->JournalHeader);
			return rc;
		}

		RC sqlite3PagerClose(Pager *pager)
		{
			_assert(assert_pager_state(pager));
		...
		Pid sqlite3PagerPagenumber(IPage *pg)
		{
...
#pragma region Main
		...
		static RC syncJournal(Pager *pager, bool newHeader)
		{
			_assert(pager->State == PAGER::WRITER_CACHEMOD || pager->State == PAGER::WRITER_DBMOD);
			_assert(assert_pager_state(pager));
			_assert(!UseWal(pager));

			RC rc = sqlite3PagerExclusiveLock(pager);
			if (rc != RC::OK) return rc;

			if (!pager->NoSync)
			{
				_assert(!pager->TempFile);
				if (pager->JournalFile->Opened && pager->JournalMode != IPager::JOURNALMODE::JMEMORY)
				{
					...
					if ((dc & VFile::IOCAP::SAFE_APPEND) == 0)
					{
						...
						if (pager->FullSync && (dc & VFile::IOCAP::SEQUENTIAL) == 0)
						{
					...
					if ((dc & VFile::IOCAP::SEQUENTIAL) == 0)
					{
						PAGERTRACE("SYNC journal of %d\n", PAGERID(pager));
						SysEx_IOTRACE("JSYNC %p\n", pager);
						rc = pager->JournalFile->Sync(pager->SyncFlags | (pager->SyncFlags == VFile::SYNC::FULL ? VFile::SYNC::DATAONLY : 0));
						if (rc != RC::OK) return rc;
					}
		...
		static RC pager_write_pagelist(Pager *pager, PgHdr *list)
		{
			...
			if (!pager->File->Opened)
			{
				_assert(pager->TempFile && rc == RC::OK);
			...
			_assert(rc != RC::OK || pager->File->Opened);
			if (rc == RC::OK && pager->DBSize > pager->DBHintSize)
			{
				int64 sizeFile = pager->PageSize * (int64)pager->DBSize;
				pager->File->FileControl(VFile::FCNTL::SIZE_HINT, &sizeFile);
			...
			while (rc == RC::OK && list)
			{
				...
				if (id <= pager->DBSize && (list->Flags & PgHdr::PGHDR::DONT_WRITE) == 0)
				{
					_assert((list->Flags & PgHdr::PGHDR::NEED_SYNC) == 0);
					if (id == 1) pager_write_changecounter(list);

					// Encode the database
					char *data; // Data to write
					CODEC2(pager, list->Data, id, 6, return RC::NOMEM, data);
					...
					if (id == 1)
						_memcpy(&pager->DBFileVersion, &data[24], sizeof(pager->DBFileVersion));
					if (id > pager->DBFileSize)
						pager->DBFileSize = id;
					pager->Stats[STAT::WRITE]++;
		...
		static RC subjournalPage(PgHdr *pg)
		{
			RC rc = RC::OK;
			Pager *pager = pg->Pager;
		...
		static RC pagerStress(void *p, IPage *pg)
		{
			Pager *pager = (Pager *)p;
			...
			if (SysEx_NEVER(pager->ErrorCode)) return RC::OK;
			if (pager->DoNotSpill) return RC::OK;
			if (pager->DoNotSyncSpill && (pg->Flags & PgHdr::PGHDR::NEED_SYNC) != 0)
				return RC::OK;
			...
			if (UseWal(pager))
			{
				...
				if (rc == RC::OK)
					rc = pagerWalFrames(pager, pg, 0, 0);
			}
			else
			{
				// Sync the journal file if required.
				if (pg->Flags & PgHdr::PGHDR::NEED_SYNC || pager->State == PAGER::WRITER_CACHEMOD)
				...
				if (SysEx_NEVER(rc == RC::OK && pg->ID > pager->DBSize && subjRequiresPage(pg)))
					rc = subjournalPage(pg);

				// Write the contents of the page out to the database file.
				if (rc == RC::OK)
				{
					_assert((pg->Flags & PgHdr::PGHDR::NEED_SYNC) == 0);
		...
		RC sqlite3PagerOpen(VFileSystem *vfs, Pager **pagerOut, const char *filename, int extraBytes, IPager::PAGEROPEN flags, VFileSystem::OPEN vfsFlags, void (*reinit)(IPage *))
		{
			...
			int journalFileSize = SysEx_ROUND8((sqlite3JournalSize(vfs) > MemoryVFile::sqlite3MemJournalSize() ? sqlite3JournalSize(vfs) : MemoryVFile::sqlite3MemJournalSize())); // Bytes to allocate for each journal fd
			...
			if (flags & IPager::PAGEROPEN::MEMORY)
			{
				memoryDB = true;
				if (filename && filename[0])
				{
					pathname = sqlite3DbStrDup(0, filename);
					if (pathname == nullptr) return RC::NOMEM;
					pathnameLength = sqlite3Strlen30(pathname);
					filename = nullptr;
				}
			}
#endif
			...
			_assert(SysEx_ROUND8(SQLITE_INT_TO_PTR(journalFileSize)));
			if (!ptr)
			{
				sqlite3DbFree(0, pathname);
				return RC::NOMEM;
			...
			pager->PCache = (PCache *)(ptr += SysEx_ROUND8(sizeof(Pager)));
			...
			{
				_assert(!pager->MemoryDB);
				rc = sqlite3PagerSetPagesize(pager, &sizePage, -1);
				ASSERTCOVERAGE(rc != RC::OK);
			}
			...
			//_assert(tempFile == IPager::LOCKINGMODE::NORMAL || tempFile == IPager::LOCKINGMODE::EXCLUSIVE);
			//_assert(IPager::LOCKINGMODE::EXCLUSIVE == 1);
			...
			else
			{
				pager->FullSync = true;
				pager->SyncFlags = VFile::SYNC::NORMAL;
				pager->WalSyncFlags = (VFile::SYNC)(VFile::SYNC::NORMAL | VFile::SYNC::WAL_TRANSACTIONS);
				pager->CheckpointSyncFlags = VFile::SYNC::NORMAL;
			}
		...
		static RC hasHotJournal(Pager *pager, bool *existsOut)
		{
			_assert(pager->UseJournal);
			_assert(pager->File->Opened);
			_assert(pager->State == PAGER::OPEN);
			...
			if (rc == RC::OK && exists)
			{
				...
				if (rc == RC::OK && !locked)
				{
					...
					if (rc == RC::OK)
						...
						{
							SysEx::BeginBenignAlloc();
							if (pagerLockDb(pager, VFile::LOCK::RESERVED) == RC::OK)
							{
								vfs->Delete(pager->Journal, false);
								if (!pager->ExclusiveMode) pagerUnlockDb(pager, VFile::LOCK::SHARED);
							}
							SysEx::EndBenignAlloc();
						}
						else
		...
		RC sqlite3PagerSharedLock(Pager *pager)
		{
			...
			_assert(pager->PCache->get_Refs()==0 );
			_assert(assert_pager_state(pager) );
			_assert(pager->State== PAGER::OPEN || pager->State == PAGER::READER);
			if (SysEx_NEVER(pager->MemoryDB && pager->ErrorCode)) return pager->ErrorCode;

			RC rc = RC::OK;
			if (!UseWal(pager) && pager->State == PAGER::OPEN)
			{
				_assert(!pager->MemoryDB);

				rc = pager_wait_on_lock(pager, VFile::LOCK::SHARED);
				if (rc != RC::OK)
				{
					_assert(pager->Lock == VFile::LOCK::NO || pager->Lock == VFile::LOCK::UNKNOWN);
					goto failed;
				}
				...
				if (pager->Lock <= VFile::LOCK::SHARED)
					rc = hasHotJournal(pager, &hotJournal);
				if (rc != RC::OK)
					goto failed;
				if (hotJournal)
				{
					...
					rc = pagerLockDb(pager, VFile::LOCK::EXCLUSIVE);
					if (rc != RC::OK)
						goto failed;
					...
					if (!pager->JournalFile->Opened)
					{
						...
						if (rc == RC::OK && exists)
						{
							...
							rc = vfs->Open(pager->Journal, pager->JournalFile, (VFileSystem::OPEN)(VFileSystem::OPEN::OREADWRITE | VFileSystem::OPEN::MAIN_JOURNAL), &fout);
							_assert(rc != RC::OK || pager->JournalFile->Opened);
					...
					{
						_assert(rc == RC::OK);
						rc = pagerSyncHotJournal(pager);
						if (rc == RC::OK)
						{
							rc = pager_playback(pager, true);
							pager->State = PAGER::OPEN;
						}
					}
					else if (!pager->ExclusiveMode)
					...
					if (rc != RC::OK)
					{
						...
						pager_error(pager, rc);
						goto failed;
					}

					_assert(pager->State == PAGER::OPEN);
					_assert((pager->Lock == VFile::LOCK::SHARED) || (pager->ExclusiveMode && pager->Lock > VFile::LOCK::SHARED));
				}

				if (!pager->TempFile && (pager->Backup || pager->PCache->get_Pages() > 0))
				{
					...
					// it can be neglected.
					Pid pages = 0;
					char dbFileVersion[sizeof(pager->DBFileVersion)];
					...
					if (pages > 0)
					{
						SysEx_IOTRACE("CKVERS %p %d\n", pager, sizeof(dbFileVersion));
						rc = pager->File->Read(&dbFileVersion, sizeof(dbFileVersion), 24);
						if (rc != RC::OK)
							goto failed;
					}
					else
					...
					if (_memcmp(pager->DBFileVersion, dbFileVersion, sizeof(dbFileVersion)) != 0)
						pager_reset(pager);
				}
				...
				rc = pagerOpenWalIfPresent(pager);
#ifndef OMIT_WAL
				_assert(pager->Wal == nullptr || rc == RC::OK);
#endif
			}
			...
			{
				_assert(rc == RC::OK);
				rc = pagerBeginReadTransaction(pager);
			}

			if (pager->State == PAGER::OPEN && rc == RC::OK)
				rc = pagerPagecount(pager, &pager->DBSize);
...
failed:
		...
		static void pagerUnlockIfUnused(Pager *pager)
		{
			if (pager->PCache->get_Refs() == 0)
				pagerUnlockAndRollback(pager);
		}

		RC sqlite3PagerAcquire(Pager *pager, Pid id, IPage **pageOut, bool noContent)
		{
			_assert(pager->State >= PAGER::READER);
			_assert(assert_pager_state(pager));
			...
			RC rc;
			if (pager->ErrorCode != RC::OK) 
				rc = pager->ErrorCode;
			else 
				rc = pager->PCache->Fetch(id, true, pageOut);
			...
			_assert((*pageOut)->Pager == pager || (*pageOut)->Pager == nullptr);
			...
			if ((*pageOut)->Pager && !noContent)
			{
				...
				_assert(id <= MAX_PID && id != IPager::MJ_PID(pager));
				pager->Stats[STAT::HIT]++;
				return RC::OK;
			}
			...
			if (id > MAX_PID || id == IPager::MJ_PID(pager))
			{
				rc = SysEx_CORRUPT_BKPT;
				goto pager_acquire_err;
			}

			if (pager->MemoryDB || pager->DBSize < id || noContent || !pager->File->Opened)
			{
				if (id > pager->MaxPid)
				{
					rc = RC::FULL;
				...
				if (noContent)
				{
					...
					SysEx::BeginBenignAlloc();
					if (id <= pager->DBOrigSize)
					{
						ASSERTONLY(rc =) pager->InJournal->Set(id);
						ASSERTCOVERAGE(rc == RC::NOMEM);
					}
					ASSERTONLY(rc =) addToSavepointBitvecs(pager, id);
					ASSERTCOVERAGE(rc == RC::NOMEM);
					SysEx::EndBenignAlloc();
				}
				_memset(pg->Data, 0, pager->PageSize);
...
pager_acquire_err:
		...
		IPage *sqlite3PagerLookup(Pager *pager, Pid id)
		{
			_assert(pager != nullptr);
			_assert(id != 0);
			_assert(pager->PCache != nullptr);
			_assert(pager->State >= PAGER::READER && pager->State != PAGER::ERROR);
			PgHdr *pg;
			pager->PCache->Fetch(id, false, &pg);
			return pg;
		}

		void sqlite3PagerUnref(IPage *pg)
		{
			if (pg)
			{
				Pager *pager = pg->Pager;
		...
		static RC pager_open_journal(Pager *pager)
		{
			_assert(pager->State == PAGER::WRITER_LOCKED);
			_assert(assert_pager_state(pager));
			...
			if( !UseWal(pager) && pager->JournalMode != IPager::JOURNALMODE::OFF)
			{
				...
				if (!pager->JournalFile->Opened)
				{
					if (pager->JournalMode == IPager::JOURNALMODE::JMEMORY)
...
#ifdef ENABLE_ATOMIC_WRITE
						rc = sqlite3JournalOpen(pager->Vfs, pager->Journal, pager->JournalFile, flags, jrnlBufferSize(pager));
#else
		...
		RC sqlite3PagerBegin(Pager *pager, int exFlag, bool subjInMemory)
		{
			if (pager->ErrorCode) return pager->ErrorCode;
			_assert(pager->State >= PAGER::READER && pager->State < PAGER::ERROR);
			pager->SubjInMemory = subjInMemory;

			RC rc = RC::OK;
			if (SysEx_ALWAYS(pager->State == PAGER::READER))
			{
				_assert(pager->InJournal == nullptr);
				...
				if (UseWal(pager))
				{
					...
					if (pager->ExclusiveMode && sqlite3WalExclusiveMode(pager->Wal, -1))
					{
						rc = pagerLockDb(pager, VFile::LOCK::EXCLUSIVE);
						if (rc != RC::OK)
							return rc;
						sqlite3WalExclusiveMode(pager->Wal, 1);
					}
					...
					rc = sqlite3WalBeginWriteTransaction(pager->Wal);
				}
				else
				{
					...
					if (rc == RC::OK && exFlag)
						rc = pager_wait_on_lock(pager, VFile::LOCK::EXCLUSIVE);
				}
				...
				if (rc == RC::OK)
				{
					...
					pager->DBHintSize = pager->DBSize;
					pager->DBFileSize = pager->DBSize;
					pager->DBOrigSize = pager->DBSize;
					pager->JournalOffset = 0;
				}

				_assert(rc == RC::OK || pager->State == PAGER::READER);
				_assert(rc != RC::OK || pager->State == PAGER::WRITER_LOCKED);
				_assert(assert_pager_state(pager));
			}
		...
		static RC pager_write(PgHdr *pg)
		{
			void *data = pg->Data;
			Pager *pager = pg->Pager;
			...
			else
			{
				...
				if (!pageInJournal(pg) && !UseWal(pager))
				{
					...
					if (pg->ID <= pager->DBOrigSize && pager->JournalFile->Opened)
					{
						...
						_assert(pg->ID != IPager::MJ_PID(pager));
		...
		RC sqlite3PagerWrite(IPage *page)
		{
			...
			if (pagePerSector > 1)
			{
				...
				for (int ii = 0; ii < pages && rc == RC::OK; ii++)
				{
					...
					{
						if (id != IPager::MJ_PID(pager))
						{
							rc = sqlite3PagerGet(pager, id, &page2);
							if (rc == RC::OK)
							{
								rc = pager_write(page2);
								if (page2->Flags & PgHdr::PGHDR::NEED_SYNC)
									needSync = true;
								sqlite3PagerUnref(page2);
							}
						}
					}
...
#ifndef DEBUG
		int sqlite3PagerIswriteable(IPage *pg)
		{
...
#define DIRECT_MODE isDirectMode
		...
		RC sqlite3PagerSync(Pager *pager)
		{
			...
			{
				_assert(!pager->MemoryDB);
				rc = pager->File->FileControl(VFile::FCNTL::SYNC_OMITTED, 0);
				if (rc == RC::NOTFOUND)
					rc = RC::OK;
			}
			return rc;
...
#pragma region Commit
		...
		RC sqlite3PagerCommitPhaseOne(Pager *pager, const char *master, int noSync)
		{
			...
			PAGERTRACE("DATABASE SYNC: File=%s zMaster=%s nSize=%d\n", pager->Filename, master, pager->DBSize);
			...
			if (pager->MemoryDB)
			{
				...
				if (pager->Backup) pager->Backup->Restart();
			}
			else
			{
				if (UseWal(pager))
				{
					PgHdr *list = pager->PCache->DirtyList();
				...
				else
				{
					...
					if (!master && pager->JournalFile->Opened &&
						...
						pager->DBSize >= pager->DBOrigSize &&
						((pg = pager->PCache->DirtyList()) == nullptr || pg->Dirty == nullptr))
					{
						...
						rc = pager_incr_changecounter(pager, true);
					}
					else
					{
						rc = sqlite3JournalCreate(pager->JournalFile);
						if (rc == RC::OK)
							rc = pager_incr_changecounter(pager, false);
					}
#else
					rc = pager_incr_changecounter(pager, false);
#endif
					...
					rc = syncJournal(pager, false);
					if (rc != RC::OK) goto commit_phase_one_exit;

					rc = pager_write_pagelist(pager, pager->PCache->DirtyList());
					if (rc != RC::OK)
					{
					...
					{
						Pid newID = pager->DBSize - (pager->DBSize == IPager::MJ_PGNO(pager) ? 1 : 0);
						_assert(pager->State == PAGER::WRITER_DBMOD);
...
commit_phase_one_exit:
		...
		RC sqlite3PagerCommitPhaseTwo(Pager *pager)
		{
			...
			if (pager->State == PAGER::WRITER_LOCKED &&
				pager->ExclusiveMode &&
				pager->JournalMode == IPager::JOURNALMODE::PERSIST)
			{
			...
			PAGERTRACE(("COMMIT %d\n", PAGERID(pager)));
			RC rc = pager_end_transaction(pager, pager->SetMaster, true);
			return pager_error(pager, rc);
		}
		...
		RC sqlite3PagerRollback(Pager *pager)
		{
			...
			if (UseWal(pager))
			{
				...
				RC rc2 = pager_end_transaction(pager, pager->SetMaster, false);
				if (rc == RC::OK) rc = rc2;
			}
			else if (!pager->JournalFile->Opened || pager->State == PAGER::WRITER_LOCKED)
			{
				PAGER state = pager->State;
				rc = pager_end_transaction(pager, false, false);
...
#pragma region Name4
		...
		{
			return pager->PCache->get_Refs();
		}

		int sqlite3PagerMemUsed(Pager *pager)
		{
			int perPageSize = pager->PageSize + pager->ExtraBytes + sizeof(PgHdr) + 5 * sizeof(void *);
			return perPageSize * pager->PCache->get_Pages() + SysEx::AllocSize(pager) + pager->PageSize;
		}

		int sqlite3PagerPageRefcount(IPage *page)
		{
		...
		{
			static int a[11];
			a[0] = pager->PCache->get_Refs();
			a[1] = pager->PCache->get_Pages();
			a[2] = pager->PCache->get_Cachesize();
			a[3] = pager->State == (PAGER::OPEN ? -1 : (int)pager->DBSize);
			a[4] = pager->State;
			a[5] = pager->ErrorCode;
			a[6] = pager->Stats[STAT::HIT];
			a[7] = pager->Stats[STAT::MISS];
			a[8] = 0;  // Used to be pager->nOvfl
			a[9] = pager->Reads;
			a[10] = pager->Stats[STAT::WRITE];
			return a;
		}
#endif

		void sqlite3PagerCacheStat(Pager *pager, int dbStatus, bool reset, int *value)
		{
			...
			//*value += pager->Stats[dbStatus - DBSTATUS::CACHE_HIT];
			//if (reset)
			//	pager->Stats[dbStatus - DBSTATUS::CACHE_HIT] = 0;
		}

		bool sqlite3PagerIsMemdb(Pager *pager)
		{
			return pager->MemoryDB;
		}

		RC sqlite3PagerOpenSavepoint(Pager *pager, int savepoints)
		{
			_assert(pager->State >= PAGER::WRITER_LOCKED);
			_assert(assert_pager_state(pager));
			...
			if (savepoints > currentSavepoints && pager->UseJournal)
			{
				...
				for (int ii = currentSavepoints; ii < savepoints; ii++)
				{
					...
					newSavepoints[ii].InSavepoint = new Bitvec(pager->DBSize);
		...
		RC sqlite3PagerSavepoint(Pager *pager, SAVEPOINT op, int savepoints)
		{
			...
			if (rc == RC::OK && savepoints < __arrayLength(pager->Savepoints))
			{
				...
				if (op == SAVEPOINT::RELEASE)
				{
					if (newLength == 0 && pager->SubJournalFile->Opened)
					{
						...
						if (VFile::IsMemoryVFile(pager->SubJournalFile))
						{
		...
		const char *sqlite3PagerFilename(Pager *pager, bool nullIfMemDb)
		{
		...
		RC sqlite3PagerMovepage(Pager *pager, IPage *pg, Pid id, bool isCommit)
		{
			_assert(pg->Refs > 0);
			_assert(pager->State == PAGER::WRITER_CACHEMOD ||
				pager->State==PAGER::WRITER_DBMOD);
			...
			{
				needSyncID = pg->ID;
				_assert(pager->JournalMode == IPager::JOURNALMODE::OFF || pageInJournal(pg) || pg->ID > pager->DBOrigSize);
				_assert(pg->Flags & PgHdr::PGHDR::DIRTY);
			}
			...
			if (pgOld)
			{
				...
				if (pager->MemoryDB)
				{
					...
					PCache::Move(pgOld, pager->DBSize + 1);
				}
				else
					PCache::Drop(pgOld);
			}
		...
		void *sqlite3PagerGetData(IPage *pg)
		{
			_assert(pg->Refs > 0 || pg->Pager->MemoryDB);
			return pg->Data;
		}

		void *sqlite3PagerGetExtra(IPage *pg)
		{
			return pg->Extra;
		}

		int sqlite3PagerLockingMode(Pager *pager, IPager::LOCKINGMODE mode)
		{
			_assert(mode == IPager::LOCKINGMODE::QUERY ||
				mode == IPager::LOCKINGMODE::NORMAL ||
				mode == IPager::LOCKINGMODE::EXCLUSIVE);
			_assert(IPager::LOCKINGMODE::QUERY < 0);
			_assert(IPager::LOCKINGMODE::NORMAL >= 0 && IPager::LOCKINGMODE::EXCLUSIVE >= 0);
		...
		IPager::JOURNALMODE sqlite3PagerSetJournalMode(Pager *pager, IPager::JOURNALMODE mode)
		{
#ifdef _DEBUG
			...
			_assert(!pager->TempFile || mode != IPager::JOURNALMODE::WAL);
		...
		IPager::JOURNALMODE sqlite3PagerGetJournalMode(Pager *pager)
		{
			return pager->JournalMode;
		}

		bool sqlite3PagerOkToChangeJournalMode(Pager *pager)
		{
			_assert(assert_pager_state(pager));
			if (pager->State >= PAGER::WRITER_CACHEMOD) return false;
			return (SysEx_NEVER(pager->JournalFile->Opened && pager->JournalOffset > 0) ? false : true);
		}
		...
		{
			if (!pager->MemoryDB && !pager->TempFile)
				pager_reset(pager);
		}
#endif
...
#pragma region Wal
		...
		RC sqlite3PagerCheckpoint(Pager *pager, int mode, int *logs, int *checkpoints)
		{
			RC rc = RC::OK;
			if (pager->Wal)
				rc = sqlite3WalCheckpoint(pager->Wal, mode, 
				pager->BusyHandler, pager->BusyHandlerArg, 
				pager->CheckpointSyncFlags, pager->PageSize, (uint8 *)pager->TmpSpace, 
				logs, checkpoints);
			return rc;
		}

		RC sqlite3PagerWalCallback(Pager *pager)
		{
		...
		bool sqlite3PagerWalSupported(Pager *pager)
		{
			...
			//return pager->ExclusiveMode || (pMethods->iVersion>=2 && pMethods->xShmMap);
		}

		static RC pagerExclusiveLock(Pager *pager)
		{
			_assert(pager->Lock == VFile::LOCK::SHARED || pager->Lock == VFile::LOCK::EXCLUSIVE);
			RC rc = pagerLockDb(pager, VFile::LOCK::EXCLUSIVE);
		...
		RC sqlite3PagerOpenWal(Pager *pager, bool *open)
		{
			_assert(assert_pager_state(pager));
			_assert(pager->State == PAGER::OPEN || open);
			_assert(pager->State == PAGER::READER || !open);
			_assert(open == nullptr || *open == false);
			_assert(open != nullptr || (!pager->TempFile && !pager->Wal));

			RC rc = RC::OK;
			if (!pager->TempFile && !pager->Wal)
			{
		...
		{
			_assert(pager->JournalMode == IPager::JOURNALMODE::WAL);
			...
			if (!pager->Wal)
			{
				...
				int logexists = 0;
				if (rc == RC::OK)
					rc = pager->Vfs->Access(pager->WalName, VFileSystem::ACCESS::EXISTS, &logexists);
			...
			if (rc == RC::OK && pager->Wal)
			{
				...
				if (rc == RC::OK)
				{
					rc = sqlite3WalClose(pager->Wal, pager->CheckpointSyncFlags, pager->PageSize, (uint8 *)pager->TmpSpace);
					pager->Wal = nullptr;
				}
			}
			return rc;
...
#pragma region Misc
		...
		{
			void *data = 0;
			CODEC2(pg->Pager, pg->Data, pg->ID, 6, return 0, data);
			return data;
		}
#endif
