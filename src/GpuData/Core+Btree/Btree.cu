...
#pragma region Struct
	...
	BtShared *_sharedCacheList = nullptr;
	bool _sharedCacheEnabled = false;
	int sqlite3_enable_shared_cache(bool enable)
	{
...
#pragma region Name3
	...
	{
		assert(cur->State == CURSOR::VALID);
		assert(cur->Key == nullptr);
		assert(cursorHoldsMutex(cur));

		RC rc = sqlite3BtreeKeySize(cur, &cur->Key);
		...
		if (cur->Pages[0]->IntKey == 0)
		{
			...
			if (key)
			{
				rc = Key(cur, 0, (int)cur->KeyLength, key);
				if (rc == RC::OK)
					cur->Key = key;
				else
					SysEx::Free(key);
	...
	static RC saveAllCursors(BtShared *bt, Pid root, BtCursor *except)
	{
		...
		for (BtCursor *p = bt->Cursor; p; p = p->Next)
		{
			if (p != except && (root == 0 || p->IDRoot == root))
				if (p->State == CURSOR::VALID)
				{
	...
	void Btree::ClearCursor(BtCursor *cur)
	{
		_assert(cursorHoldsMutex(cur));
		SysEx::Free(cur->Key);
		cur->Key = nullptr;
	...
	static RC btreeMoveto(BtCursor *cur, const void *key, int64 keyLength, int bias, int *res)
	{
		...
		if (key)
		{
			...
			idxKey = sqlite3VdbeAllocUnpackedRecord(cur->KeyInfo, space, sizeof(space), &free);
			if (idxKey == nullptr) return RC::NOMEM;
			sqlite3VdbeRecordUnpack(cur->KeyInfo, (int)keyLength, key, idxKey);
		}
		else
			idxKey = nullptr;
		RC rc = sqlite3BtreeMovetoUnpacked(cur, idxKey, keyLength, bias, res);
		if (free)
	...
	{
		_assert(cursorHoldsMutex(cur));
		_assert(cur->State >= CURSOR::REQUIRESEEK);
		...
		if (rc == RC::OK)
		{
			SysEx::Free(cur->Key);
			cur->Key = nullptr;
			_assert(cur->State == CURSOR::VALID || cur->State == CURSOR::INVALID);
		}
		return rc;
	}

#define restoreCursorPosition(p) (p->State >= CURSOR::REQUIRESEEK ? btreeRestoreCursorPosition(p) : RC::OK)

	RC Btree::CursorHasMoved(BtCursor *cur, bool *hasMoved)
	{
...
#pragma region Parse Cell
	...
	static void ptrmapPut(BtShared *bt, Pid key, PTRMAP type, Pid parent, RC *rcRef)
	{
		...
		Pid ptrmapIdx = PTRMAP_PAGENO(pBt, key); // The pointer map page number
		...
		uint8 *ptrmap = (uint8 *)Pager::GetData(page);// The pointer map data
...
ptrmap_exit:
	...
	static RC ptrmapGet(BtShared *bt, Pid key, PTRMAP *type, Pid *id)
	{
		_assert(MutexEx::Held(bt->Mutex));
		...
		int offset = PTRMAP_PTROFFSET(ptrmap, key); // Offset of entry in pointer map
		if (offset < 0)
		{
			Pager::Unref(page);
		...
		*type = (PTRMAP)ptrmap[offset];
		...
		Pager::Unref(page);
		if (*type < 1 || *type > 5) return SysEx_CORRUPT_BKPT;
		return RC::OK;
	}
...
#define findCell(P,I) ((P)->Data + ((P)->MaskPage & ConvertEx::Get2(&(P)->CellIdx[2*(I)])))
	...
	static void btreeParseCellPtr(MemPage *page, uint8 *cell, CellInfo *info)
	{
		_assert(MutexEx::Held(page->Bt->Mutex));

		info->Cell = cell;
		_assert(page->Leaf == 0 || page->Leaf == 1);
		...
		{
			if (page->HasData)
				n += ConvertEx::GetVariant4(&cell[n], &payloadLength);
			else
				payloadLength = 0;
			n += ConvertEx::GetVariant(&cell[n], (uint64 *)&info->Key);
...
#define parseCell(page, cell, info) btreeParseCellPtr((page), findCell((page), (cell)), (info))
	static void btreeParseCell(MemPage *page, int cell, CellInfo *info) { parseCell(page, cell, info); }
	static uint16 cellSizePtr(MemPage *page, uint8 *cell)
	{
#ifdef _DEBUG
		...
		{
			if (page->HasData)
				iter += ConvertEx::GetVariant4(iter, &size);
			else
				size = 0;
...
#ifdef _DEBUG
	static uint16 cellSize(MemPage *page, int cell)
	{
...
#ifndef OMIT_AUTOVACUUM
	static void ptrmapPutOvflPtr(MemPage *page, uint8 *cell, RC *rcRef)
	{
		if (*rcRef != RC::OK) return;
...
#pragma region Allocate / Defragment
	...
	static RC allocateSpace(MemPage *page, int bytes, int *idx)
	{
		...
		assert(top+bytes <= (int)page->Bt->UsableSize);
		*idx = top;
		return RC::OK;
	}
	...
	static RC freeSpace(MemPage *page, int start, int size)
	{
		...
		if (page->Bt->BtsFlags & BTS::SECURE_DELETE) // Overwrite deleted information with zeros when the secure_delete option is enabled
	...
	static RC decodeFlags(MemPage *page, int flagByte)
	{
		...
		_assert(MutexEx::Held(page->Bt->Mutex));
		page->Leaf = (uint8)(flagByte >> 3); _assert(PTF_LEAF == 1 << 3);
		flagByte &= ~PTF_LEAF;
	...
	static RC btreeInitPage(MemPage *page)
	{
		_assert(page->Bt != nullptr);
		_assert(MutexEx::Held(page->Bt->Mutex));
		_assert(page->ID == Pager::GetPageID(page->DBPage));
	...
	static void zeroPage(MemPage *page, int flags)
	{
		BtShared *bt = page->Bt;
		unsigned char *data = page->Data;
		_assert(Pager::GetPageID(page->DBPage) == page->ID);
		...
		page->OverflowsUsed = 0;
		_assert(bt->PageSize>=512 && pBt->pageSize<=65536 );
		page->MaskPage = (uint16)(pBt->pageSize - 1);
		page->Cells = 0;
		page->IsInit = true;
	}
...
#pragma region Page
	...
	static RC btreeGetPage(BtShared *bt, Pid id, MemPage **page, bool noContent)
	{
		_assert(MutexEx::Held(bt->Mutex));
		IPage *dbPage;
	...
	Pid Btree::LastPage()
	{
		_assert(HoldsMutex(p));
		_assert(((Bt->Pages) & 0x8000000) == 0);
		return btreePagecount(Bt);
	}

	static RC getAndInitPage(BtShared *bt, Pid id, MemPage **page)
	{
		_assert(MutexEx::Held(bt->Mutex));
...
#pragma region Open / Close
	...
	static int btreeInvokeBusyHandler(void *arg)
	{
	...
	RC Btree::Open(VFileSystem *vfs, const char *filename, Context *ctx, Btree **btree, OPEN flags, VFileSystem::OPEN vfsFlags)
	{
		...
		const int memoryDB = (filename && strcmp(filename, ":memory:") == 0) ||
		...
		_assert((flags & 0xff) == flags); // flags fit in 8 bits

		// Only a BTREE_SINGLE database can be BTREE_UNORDERED
		_assert((flags & OPEN::UNORDERED) == 0 || (flags & OPEN::SINGLE) != 0);

		// A BTREE_SINGLE database is always a temporary and/or ephemeral
		_assert((flags & OPEN::SINGLE) == 0 || isTempDb);
		...
		Btree *p = SysEx::Alloc(sizeof(Btree), true); // Handle to return
		if (!p)
			return RC::NOMEM;
		...
		if (!tempDB && (!memoryDB || (vfsFlags & VFileSystem::OPEN::URI) != 0))
			if (vfsFlags & VFileSystem::OPEN::SHAREDCACHE)
			{
				...
				char *fullPathname = SysEx::Alloc(fullPathnameLength);
				p->Sharable = true;
				if (!fullPathname)
				{
					SysEx::Free(p);
				...
				{
					_assert(bt->Refs > 0);
					if (strcmp(fullPathname, bt->Pager->get_Filename(false) == 0) && bt->Pager->get_Vfs() == vfs)
					{
						for (int i = __arrayLength(ctx->DBs) - 1; db >= 0; db--)
						{
							Btree *existing = ctx->DBs[i].Bt;
							if (existing && existing->Bt == bt)
							{
								Mutex::Leave(mutexShared);
								Mutex::Leave(mutexOpen);
								SysEx::Free(fullPathname);
								SysEx::Free(p);
		...
		if (bt == nullptr)
		{
			...
			bt->OpenFlags = flags;
			bt->Ctx = ctx;
			pt->Pager->SetBusyhandler(btreeInvokeBusyHandler, bt);
			p->Bt = bt;
			...
			if (bt->PageSize < 512 || bt->PageSize > MAX_PAGE_SIZE || ((bt->PageSize - 1) & bt->PageSize) != 0)
			{
				...
				{
					bt->AutoVacuum = (DEFAULT_AUTOVACUUM);
...
#ifndef OMIT_AUTOVACUUM
				pBt->AutoVacuum = ConvertEx::Get4(&dbHeader[36 + 4 * 4]);
				pBt->IncrVacuum = ConvertEx::Get4(&dbHeader[36 + 7 * 4]);
#endif
			}
...
#if THREADSAFE
				mutexShared = MutexEx::Alloc(MutexEx::MUTEX:::STATIC_MASTER);
				bt->Mutex = MutexEx::Alloc(SysEx::MUTEX::FAST);
#endif
				MutexEx::Enter(mutexShared);
		...
		{
			Btree *sib;
			for (int i = 0; i < __arrayLength(ctx->DBs); i++)
				if ((sib = ctx->DBs[i].Bt) != 0 && sib->Sharable)
				{
					while (sib->Prev) { sib = sib->pPrev; }
					if (p->Bt < sib->Bt)
					{
						p->Next = sib;
						p->Prev = nullptr;
...
btree_open_out:
	...
	static bool removeFromSharingList(BtShared *bt)
	{
		...
		MutexEx master;
#if THREADSAFE
		master = MutexEx::Alloc(SQLITE_MUTEX_STATIC_MASTER);
#endif
		bool removed = false;
		...
		if (bt->Refs <= 0)
		{
			...
			else
			{
				...
				while (SysEx::ALWAYS(list) && list->Next != bt)
					list = list->Next;
	...
	{
		if (!bt->TmpSpace)
			bt->TmpSpace = sqlite3PageMalloc(bt->PageSize);
	}

	static void freeTempSpace(BtShared *bt)
	{
		sqlite3PageFree(bt->TmpSpace);
		bt->TmpSpace = nullptr;
	}
	...
	RC Btree::Close()
	{
		...
		sqlite3BtreeRollback(this, RC::OK);
...
#pragma region Settings
	...
	RC Btree::SetCacheSize(int maxPage)
	{
		_assert(MutexEx::Held(Ctx->Mutex));
		Enter();
	...
	RC Btree::SetSafetyLevel(int level, bool fullSync, bool ckptFullSync)
	{
		_assert(MutexEx::Held(Ctx->Mutex));
	...
	RC Btree::SetPageSize(int pageSize, int reserves, bool fix)
	{
		_assert(reserves >= -1 && reserves <= 255);
		Enter();
		BtShared *bt = Bt;
		if (bt->BtsFlags & BTS::PAGESIZE_FIXED)
		{
			Leave();
			return RC::READONLY;
		}
	...
	int Btree::GetPageSize()
	{
		return Bt->PageSize;
	}

#if defined(HAS_CODEC) || defined(_DEBUG)
	int Btree::GetReserveNoMutex()
	{
		_assert(MutexEx::Held(Bt->Mutex));
	...
	bool Btree::SecureDelete(bool newFlag)
	{
		Enter();
		Bt->BtsFlags &= ~BTS::SECURE_DELETE;
	...
	RC Btree::SetAutoVacuum(Btree::AUTOVACUUM autoVacuum)
	{
		...
		else
		{
			...
			bt->IncrVacuum = (autoVacuum == Btree::AUTOVACUUM::INCR);
		}
		Leave();
		return rc;
#endif
	...
	Btree::AUTOVACUUM Btree::GetAutoVacuum()
	{
#ifdef OMIT_AUTOVACUUM
...
#pragma region Lock / Unlock
	...
	static RC lockBtree(BtShared *bt)
	{
		...
		Pid pages = pagesHeader = ConvertEx::Get4(28 + (uint8 *)page1->Data); // Number of pages in the database
		...
		// page pointer.
		pt->MaxLocal = (uint16)((bt->UsableSize - 12) * 64 / 255 - 23);
		pt->MinLocal = (uint16)((bt->UsableSize - 12) * 32 / 255 - 23);
		pt->MaxLeaf = (uint16)(bt->UsableSize - 35);
		pt->MinLeaf = (uint16)((bt->UsableSize - 12) * 32 / 255 - 23);
		if (bt->MaxLocal > 127)
...
#pragma region NewDB
	...
	static RC newDatabase(BtShared *bt)
	{
		...
		MemPage *p1 = bt->Page1;
		_assert(p1 != nullptr);
		unsigned char *data = p1->Data;
		RC rc = Pager::Write(p1->DBPage);
		if (rc) return rc;
		_memcpy(data, _magicHeader, sizeof(_magicHeader));
...
#pragma region Transactions
	...
	RC Btree::BeginTrans(int wrflag)
	{
		Enter();
		btreeIntegrity(this);
		...
		if (InTrans == TRANS::WRITE || (InTrans == TRANS::READ && !wrflag))
			goto trans_begun;
		_assert(IfNotOmitAV(bt->DoTruncate) == 0);
		...
		{
			for (BtLock *iIter = bt->Lock; iter; iter = iter->Next)
				if (iter->Btree != this)
				{
		...
		if (rc == RC::OK)
		{
			...
			InTrans = (wrflag ? TRANS::WRITE : TRANS::READ);
...
#pragma region Autovacuum
	...
	static RC setChildPtrmaps(MemPage *page)
	{
		...
		int cells = page->Cells; // Number of cells in page pPage
...
set_child_ptrmaps_out:
	...
	static int modifyPagePointer(MemPage *page, Pid from, Pid to, PTRMAP type)
	{
		...
		{
			bool isInitOrig = pPage->IsInit;

			btreeInitPage(page);
			int cells = pPage->Cells;
			...
			if (i == cells)
			{
				if (type != PTRMAP::BTREE || ConvertEx::Get4(&pPage->Data[page->HdrOffset + 8]) != fFrom)
					return SysEx_CORRUPT_BKPT;
	...
	{
		_assert(type == PTRMAP::OVERFLOW2 || type == PTRMAP::OVERFLOW1 || type == PTRMAP::BTREE || type == PTRMAP::ROOTPAGE);
		_assert(MutexEx::Held(bt->Mutex));
		_assert(page->Bt == bt);
		...
		else
		{
			...
			if (nextOvfl != 0)
			{
				ptrmapPut(bt, nextOvfl, PTRMAP::OVERFLOW2, freePageID, &rc);
				if (rc != RC::OK)
					return rc;
			}
		}
	...
	static int incrVacuumStep(BtShared *bt, Pid fins, Pid lastPageID, bool commit)
	{
		...
		{
			Pif freesList = ConvertEx::Get4(&bt->Page1->Data[36]); // Number of pages still on the free-list
			if (freesList == 0)
				return RC::DONE;

			PTRMAP type;
			Pid ptrPageID;
			RC rc = ptrmapGet(pBt, lastID, &type, &ptrPageID);
			if (rc != RC::OK)
				return rc;
			if (type == PTRMAP::ROOTPAGE)
				return SysEx_CORRUPT_BKPT;
	...
	RC Btree::IncrVacuum()
	{
		...
		Enter();
		_assert(bt->InTransaction == TRANS::WRITE && InTrans == TRANS::WRITE);
		RC rc;
		if (!bt->AutoVacuum)
			rc = RC::DONE;
		else
...
#pragma region Commit / Rollback
	...
	RC Btree::CommitPhaseOne(const char *master)
	{
		RC rc = RC::OK;
	...
	RC Btree::CommitPhaseTwo(bool cleanup)
	{
		...
		{
			BtShared *bt = Bt;
			_assert(bt->InTransaction == TRANS::WRITE);
	...
	void Btree::TripAllCursors(RC errCode)
	{
		Enter();
		for (BtCursor *p = Bt->Cursor; p; p = p->Next)
		{
	...
	RC Btree::Rollback(RC tripCode)
	{
		...
		if (InTrans == TRANS::WRITE)
		{
			...
			if (btreeGetPage(bt, 1, &page1, false) == RC::OK)
			{
				...
				if (pages == 0) bt->Pager->Pagecount(&pages);
	...
	RC Btree::BeginStmt(int statements)
	{
		BtShared *bt = p->Bt;
		Enter();
		_assert(InTrans == TRANS::WRITE);
		_assert((bt->BtsFlags & BTS::READ_ONLY) == 0);
	...
	RC Btree::Savepoint(IPager::SAVEPOINT op, int savepoints)
	{
		RC rc = RC::OK;
		if (InTrans == TRANS::WRITE)
		{
			BtShared *bt = Bt;
			_assert(op == IPager::SAVEPOINT::RELEASE || op == IPager::SAVEPOINT::ROLLBACK);
			_assert(savepoints >= 0 || (savepoints == -1 && op == IPager::SAVEPOINT::ROLLBACK));
			Enter();
...
#pragma region Cursors
	...
	static RC btreeCursor(Btree *p, int table, int wrFlag, struct KeyInfo *keyInfo, BtCursor *cur)
	{
		BtShared *bt = p->pBt; // Shared b-tree handle

		_assert(HoldsMutex(p));
		_assert(wrFlag == 0 || wrFlag == 1);
		...
		_assert(hasSharedCacheTableLock(p, table, keyInfo != nullptr, wrFlag + 1));
		_assert(wrFlag == 0 || !hasReadConflicts(p, table));

		// Assert that the caller has opened the required transaction.
		_assert(p->InTrans > TRANS::NONE);
		_assert(wrFlag == 0 || p->InTrans == TRANS::WRITE);
	...
	RC Btree::Cursor(int table, int wrFlag, struct KeyInfo *keyInfo, BtCursor *cur)
	{
		Enter();
	...
	int Btree::CursorSize()
	{
		return SysEx::ROUND8(sizeof(BtCursor));
	}

	void Btree::CursorZero(BtCursor *p)
	{
		_memset(p, 0, offsetof(BtCursor, page));
	}

	void Btree::SetCachedRowID(BtCursor *cur, int64 rowid)
	{
		for (BtCursor *p = cur->Bt->Cursor; p; p = p->Next)
			if (p->IDRoot == cur->IDRoot) p->CachedRowID = rowid;
		_assert(cur->CachedRowID == rowid);
	}

	int64 Btree::GetCachedRowID(BtCursor *cur)
	{
		return cur->CachedRowID;
	}

	RC Btree::CloseCursor(BtCursor *cur)
	{
	...
	static void assertCellInfo(BtCursor *cur)
	{
		...
		btreeParseCell(cur->Pages[page], cur->Idxs[pPage], &info);
...
#define assertCellInfo(x)
	...
	static void getCellInfo(BtCursor *cur)
	{
		...
		{
			int page = cur->Page;
			btreeParseCell(cur->Pages[page], cur->Idxs[pPage], &cur->info);
			cur->ValidNKey = 1;
		}
		else
			assertCellInfo(cur);
...
#define getCellInfo(cur) \
...
#ifdef _DEBUG
	bool Btree::CursorIsValid(BtCursor *cur)
	{
		return cur && cur->State == CURSOR::VALID;
	}
#endif

	RC Btree::KeySize(BtCursor *cur, int64 *size)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(cur->State == CURSOR::INVALID || cur->State == CURSOR::VALID);
	...
	RC Btree::DataSize(BtCursor *cur, uint32 *size)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(cur->State == CURSOR::VALID);
		getCellInfo(cur);
		*size = cur->Info.Data;
		return RC::OK;
	}
...
#pragma region Payload / Overflow
	...
	static int getOverflowPage(BtShared *bt, Pid ovfl, MemPage **pageOut, Pid *idNextOut)
	{
		Pid next = 0;
	...
	static RC copyPayload(void *payload, void *buf, int bytes, int op, IPage *dbPage)
	{
		if (op)
		{
	...
	static RC accessPayload(BtCursor *cur, uint32 offset, uint32 amount, unsigned char *buf, int op)
	{
		MemPage *page = cur->Pages[cur->Page]; // Btree page of current entry

		_assert(page);
		_assert(cur->State == CURSOR::VALID);
		_assert(cur->Idxs[cur->Page] < page->Cells);
		_assert(cursorHoldsMutex(cur));

		getCellInfo(cur);
		unsigned char *payload = cur->Info.Cell + cur->Info.Header;
		uint32 key = (page->IntKey ? 0 : (int)cur->Info.Key);

		BtShared *bt = cur->Bt; // Btree this cursor belongs to
		if (SysEx::NEVER(offset + amount > key + cur->Info.Data) || &payload[cur->Info.Local] > &page->Data[bt->UsableSize])
		...
		if (rc == RC::OK && amount > 0)
		{
			...
			if (cur->IsIncrblobHandle && !cur->Overflows)
			{
				...
				if (SysEx::ALWAYS(ovfl) && !cur->Overflows)
					rc = RC::NOMEM;
			}
			...
			for ( ; rc == RC::OK && amount > 0 && nextPage; idx++)
			{
				...
				else
				{
					...
					{
						IPage *dbPage;
						rc = bt->Pager->Aquire(nextPage, &dbPage, false);
						if (rc == RC::OK)
						{
	...
	RC Btree::Key(BtCursor *cur, uint32 offset, uint32 amount, void *buf)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(cur->State == CURSOR::VALID);
		_assert(cur->Page >= 0 && cur->Pages[cur->Page]);
		_assert(cur->Idx[cur->Page] < cur->Pages[cur->Page]->Cells);
		return accessPayload(cur, offset, amount, (unsigned char*)buf, 0);
	}

	RC Btree::Data(BtCursor *cur, uint32 offset, uint32 amount, void *buf)
	{
#ifndef OMIT_INCRBLOB
		...
		if (rc == RC::OK)
		{
			_assert(cur->State == CURSOR::VALID);
			_assert(cur->Page >= 0 && cur->Pages[cur->Page]);
			_assert(cur->Idxs[cur->Page] < cur->Pages[cur->Page]->Cells);
			rc = accessPayload(cur, offset, amount, buf, 0);
		}
		return rc;
	}

	static const unsigned char *fetchPayload(BtCursor *cur, int *amount, bool skipKey)
	{
		_assert(cur != nullptr && cur->Page >= 0 && cur->Pages[cur->Page]);
		_assert(cur->State == CURSOR::VALID);
		_assert(cursorHoldsMutex(pCur) );
		MemPage *page = cur->Pages[cur->Page];
		_assert(cur->Idxs[cur->Page] < page->Cells);
		if (SysEx::NEVER(cur->Info.Sizes == 0))
			btreeParseCell(cur->Pages[cur->Page], cur->Idxs[cur->Page], &cur->Info);
		unsigned char *payload = cur->Info.Cell + cur->Info.Header;
	...
	const void *Btree::KeyFetch(BtCursor *cur, int *amount)
	{
	...
	const void *Btree::DataFetch(BtCursor *cur, int *amount)
	{
		const void *p = 0;
		_assert(MutexEx::Held(cur->Btree->Ctx->Mutex));
		_assert(cursorHoldsMutex(cur));
...
#pragma region Move Cursor
	...
	static RC moveToChild(BtCursor *cur, uint32 newID)
	{
		_assert(cursorHoldsMutex(pCur) );
		_assert(cur->State==CURSOR_VALID );
		_assert(cur->Page < BTCURSOR_MAX_DEPTH );
...
#define assertParentIndex(x,y,z) 
...
#endif
		ASSERTCOVERAGE(cur->Idxs[cur->Page - 1] > cur->Pages[cur->Page - 1]->Cells);
	...
	{
		_assert(cursorHoldsMutex(pCur) );
		_assert(CURSOR::INVALID < CURSOR::REQUIRESEEK);
		_assert(CURSOR::VALID < CURSOR::REQUIRESEEK);
		_assert(CURSOR::FAULT > CURSOR::REQUIRESEEK);
		if (cur->State >= CURSOR::REQUIRESEEK)
		{
			if (cur->State == CURSOR::FAULT)
			{
				_assert(cur->SkipNext != RC::OK);
				return cur->SkipNext;
			}
			ClearCursor(cur);
		}
		...
		{
			rc = getAndInitPage(bt, cur->IDRoot, &cur->Pages[0]);
			if (rc != RC::OK)
			{
				cur->State = CURSOR::INVALID;
				return rc;
			...
			_assert(cur->Pages[0]->IntKey == 1 || cur->Pages[0]->IntKey == 0);
	...
	static RC moveToLeftmost(BtCursor *cur)
	{
		...
		MemPage *page;
		RC rc = RC::OK;
		while (rc == RC::OK && !(page = cur->Pages[cur->Page])->Leaf)
		{
			_assert(cur->Idxs[cur->Page] < page->Cells);
			Pid id = ConvertEx::Get4(findCell(page, cur->Idxs[cur->Page]));
	...
	static RC moveToRightmost(BtCursor *cur)
	{
		...
		if (rc == RC::OK)
		{
			cur->Idxs[cur->Page] = page->Cells - 1;
			cur->Info.Size = 0;
			cur->ValidNKey = 0;
		}
		return rc;
	...
	RC Btree::First(BtCursor *cur, int *res)
	{
		_assert(cursorHoldsMutex(cur) );
		_assert(MutexEx::Held(cur->Btree->Ctx->Mutex));
		...
		if (rc == RC::OK)
		{
			if (cur->State == CURSOR::INVALID)
			{
				_assert(cur->IDRoot == 0 || cur->Pages[cur->Page]->Cells == 0);
				*res = 1;
			}
			else
			{
				_assert(cur->Pages[cur->Page]->Cells > 0);
				*res = 0;
	...
	RC Btree::Last(BtCursor *cur, int *res)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(MutexEx::Held(cur->Btree->Ctx->Mutex));

		// If the cursor already points to the last entry, this is a no-op.
		if (cur->State == CURSOR::VALID && cur->AtLast)
		{
#ifdef _DEBUG
			// This block serves to assert() that the cursor really does point to the last entry in the b-tree.
			for (int ii = 0; ii < cur->Page; ii++)
				_assert(cur->Idxs[ii] == cur->Pages[ii]->Cells);
			_assert(cur->Idxs[cur->Page] == cur->Pages[cur->Page]->Cells - 1);
			_assert(cur->Pages[cur->Page]->Leaf);
#endif
			return RC::OK;
		}
	...
	RC Btree::MovetoUnpacked(BtCursor *cur, UnpackedRecord *idxKey, int64 intKey, int biasRight, int *res)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(MutexEx::Held(cur->Btree->Ctx->Mutex));
		_assert(res);
		...
		if (rc)
			return rc;
		_assert(cur->IDRoot == 0 || cur->Pages[cur->Page]);
		_assert(cur->IDRoot == 0 || cur->Pages[cur->Page]->IsInit);
		_assert(cur->State == CURSOR::INVALID || cur->Pages[cur->Page]->Cells > 0);
		if (cur->State == CURSOR::INVALID)
		{
			*res = -1;
			_assert(cur->IDRoot == 0 || cur->Pages[cur->Page]->Cells == 0);
		...
		for (; ; )
		{
			...
			if (biasRight)
				cur->Idxs[cur->Page] = (uint16)(idx = upr);
			else
				cur->Idxs[cur->Page] = (uint16)(idx = (upr + lwr) / 2);
			for (; ; )
			{
				_assert(idx == cur->Idxs[cur->Page]);
				cur->Info.Size = 0;
				...
				else
				{
					...
					if (cellLength <= page->Max1bytePayload /* && (cell + cellLength) < page->DataEnd */)
					{
						...
						c = sqlite3VdbeRecordCompare(cellLength, (void *)&cell[1], idxKey);
					}
					else if (!(cell[1] & 0x80) && (cellLength = ((cellLength & 0x7f) << 7) + cell[1]) <= page->MaxLocal /* && (cell + cellLength + 2) <= page->DataEnd */)
					{
						...
						c = sqlite3VdbeRecordCompare(cellLength, (void *)&cell[2], idxKey);
					}
					else
					{
				...
				if (lwr > upr)
					break;
				cur->Idxs[cur->Page] = (uint16)(idx = (lwr + upr) / 2);
			}
			...
			if (page->Leaf)
				...
				chldPg = ConvertEx::Get4(findCell(page, lwr));
			if (chldPg == 0)
			{
				_assert(cur->Idxs[cur->Page] < cur->Pages[cur->Page]->Cells);
				*res = c;
...
moveto_finish:
	...
	RC Btree::Next(BtCursor *cur, int *res)
	{
		_assert(cursorHoldsMutex(cur));
		RC rc = restoreCursorPosition(cur);
		if (rc != RC::OK)
	...
	RC Btree::Previous(BtCursor *cur, int *res)
	{
		assert(cursorHoldsMutex(cur));
		RC rc = restoreCursorPosition(cur);
		if (rc != RC::OK)
...
#pragma region Allocate Page
	...
	static int allocateBtreePage(BtShared *bt, MemPage **page, Pid *id, Pid nearby, BTALLOC mode)
	{
		_assert(MutexEx::Held(bt->Mutex));
		_assert(mode == BTALLOC::ANY || (nearby > 0 && IfNotOmitAV(bt->AutoVacuum)));
		MemPage *page1 = bt->Page1;
...
end_allocate_page:
	...
	static RC freePage2(BtShared *bt, MemPage *memPage, Pid pageID)
	{
		...
		Pid frees = ConvertEx::Get4(&page1->Data[36]); // Initial number of pages on free-list
		...
		if (ISAUTOVACUUM)
		{
			ptrmapPut(bt, pageID, PTRMAP::FREEPAGE, 0, &rc);
			if (rc) goto freepage_out;
		}
		...
		if (frees != 0)
		{
			...
			if (leafs < (uint32)bt->UsableSize / 4 - 8)
			{
				...
				if (rc == RC::OK)
				{
					...
					if (page && (bt->BtsFlags & BTS::SECURE_DELETE) == 0)
...
freepage_out:
	...
	static void freePage(MemPage *page, RC *rc)
	{
		if ((*rc) == RC::OK)
			*rc = freePage2(page->Bt, page, page->ID);
	}

	static RC clearCell(MemPage *page, unsigned char *cell)
	{
		_assert(MutexEx::Held(page->Bt->Mutex));
		CellInfo info;
		btreeParseCellPtr(page, cell, &info);
		...
		while (ovfls--)
		{
			...
			if ((ovfl || (ovfl = btreePageLookup(bt, ovflID)) != RC.OK) && Pager::PageRefs(ovfl->DBPage) != 1)
			{
	...
	static int fillInCell(MemPage *page, unsigned char *cell, const void *key, int64 keyLength, const void *data, int dataLength, int zeros, int *sizeOut)
	{
		BtShared *bt = page->Bt;
		_assert(MutexEx::Held(bt->Mutex));
		...
		{ 
			if (SysEx::NEVER(keyLength > 0x7fffffff || key == nullptr))
				return SysEx_CORRUPT_BKPT;
		...
		while (payloadLength > 0)
		{
			if (spaceLeft == 0)
			{
				...
				if (bt->AutoVacuum)
					...
					{
						PTRMAP type = (idPtrmap ? PTRMAP::OVERFLOW2 : PTRMAP::OVERFLOW1);
	...
	static void dropCell(MemPage *page, int idx, int sz, RC *rcRef)
	{
		if (*rcRef) return;

		_assert(idx >= 0 && idx < page->Cells);
		_assert(sz == cellSize(page, idx) );
		_assert(Pager::Iswriteable(page->DBPage));
		_assert(MutexEx::Held(page->Bt->Mutex));
		...
		if (pc < (uint32)ConvertEx::Get2(&data[hdr + 5]) || pc + sz > page->Bt->UsableSize)
		{
			*rcRef = SysEx_CORRUPT_BKPT;
			return;
		}
		RC rc = freeSpace(page, pc, sz);
		if (rc)
		{
			*rcRef = rc;
			return;
	...
	static void insertCell(MemPage *page, int i, uint8 *cell, int sz, uint8 *temp, Pid childID, RC *rcRef)
	{
		if (*rcRef) return;

		_assert(i >= 0 && i <= page->Cells + page->OverflowUsed);
		...
		_assert(sz == cellSizePtr(page, cell) || (sz == 8 && childID > 0));
		int skip = (childID ? 4 : 0);
		if (page->Overflows || sz + 2 > page->Frees)
		{
			if (temp)
			{
				_memcpy(temp + skip, cell + skip, sz - skip);
				cell = temp;
			}
			if (childID)
		...
		else
		{
			...
			rc = allocateSpace(page, sz, &idx);
			if (rc) { *rcRef = rc; return; }
			// The allocateSpace() routine guarantees the following two properties if it returns success
			_assert(idx >= end + 2);
			_assert(idx + sz <= (int)page->Bt->UsableSize);
			page->Cells++;
			page->Frees -= (uint16)(2 + sz);
			_memcpy(&data[idx + skip], cell + skip, sz - skip);
			if (childID)
			...
			if (page->Bt->AutoVacuum)
			{
				...
				ptrmapPutOvflPtr(page, cell, rcOut);
			}
#endif
		}
	}

	static void assemblePage(MemPage *page, int cells, uint8 **cellSet, uint16 *sizes)
	{
		...
		page->Free -= (cells * 2 + usable - cellbody);
...
#define NB (NN * 2 + 1)		// Total pages involved in the balance
	...
	static RC balance_quick(MemPage *parent, MemPage *page, uint8 *space)
	{
		...
		if (rc == RC::OK)
		{
			...
			if (ISAUTOVACUUM)
			{
				ptrmapPut(bt, newPageID, PTRMAP::BTREE, parent->ID, &rc);
		...
		for (int i = 0; i < pages; i++)
		{
			...
			for (int j = 0; j < page->Cells; j++)
			{
				...
				if (info.Overflow)
				{
					...
					_assert(n == page->ID && e == PTRMAP::OVERFLOW1);
				}
				if (!page->Leaf)
				{
					Pid child = ConvertEx::Get4(z);
					ptrmapGet(bt, child, &e, &n);
					_assert(n == page->ID && e == PTRMAP::BTREE);
				}
			}
			if (!page->Leaf)
			{
	...
	static void copyNodeContent(MemPage *from, MemPage *to, RC *rcRef)
	{
		if ((*rcRef) == RC::OK)
		{
			...
			_assert(ConvertEx::Get2(&fromdata[fromHdr + 5]) <= (int)bt->UsableSize);
...
#pragma optimize("", off)
#endif
	static int balance_nonroot(MemPage *parent, int parentIdx, uint8 *ovflSpace, bool isRoot, bool bulk)
	{
		...
		if ((i + nxDiv - parent->Overflows) == parent->Cells)
			right = &parent->Data[parent->HdrOffset + 8];
		else
		...
		int countNew[NB + 2]; // Index in aCell[] of cell after i-th page
		int sizeNew[NB + 2]; // Combined size of cells place on i-th page
		while (true)
		{
			...
			else
			{
				...
				if (bt->BtsFlags & BTS::SECURE_DELETE)
				{
					...
					{
						_memcpy(&ovflSpace[off], divs[i], szNew[i]);
						apDiv[i] = &ovflSpace[apDiv[i] - parent->Data];
					}
				}
		...
		uint8 **cell = sqlite3ScratchMalloc(sizeScratch); // All cells begin balanced
		if (cell == nullptr)
		{
			rc = RC::NOMEM;
			goto balance_cleanup;
		...
		u16 *sizeCell = (uint16 *)&apCell[maxCells]; // Local size of all cells in apCell[]
		int space1Idx = 0; // First unused byte of aSpace1[]
		byte *space1 = (uint8 *)&sizeCell[maxCells]; // Space for copies of dividers cells
		_assert(EIGHT_BYTE_ALIGNMENT(space1));
		...
		for (i = 0; i < newPagesUsed; i++)
		{
			...
			{
				assert(j < maxCells);
				uint8 *pCell = apCell[j];
				int sz = szCell[j] + leafCorrection;
				uint8 *pTemp = &aOvflSpace[iOvflSpace];
				if (!newPage->Leaf)
				...
				iOvflSpace += sz;
				assert(sz <= bt->MaxLocal + 23);
				assert(iOvflSpace <= (int)bt->PageSize);
		...
		if ((pageFlags & PTF_LEAF) == 0)
		{
			...
			memcpy(&newPages[newPagesUsed - 1]->Data[8], child, 4);
		}
		...
		else if (bt.AutoVacuum)
		{
			...
			for (i = 0; i < cells; i++)
			{
				...
				while (i == nextOldID)
				{
					...
					if (oldPage->Overflows)
					{
						...
						overflowID = i + !leafData + oldPage->OvflIdx[0];
					}
					isDivider = !leafData;
				}

				_assert(overflows > 0 || overflowID < i);
				_assert(overflows < 2 || oldPages->OvflIdxs[0] == oldPages->OvflIdxs[1] - 1);
				_assert(overflows < 3 || oldPages->OvflIdxs[1] == oldPages->OvflIdxs[2] - 1);
				if (i == overflowID)
				{
					isDivider = true;
					if (--overflows > 0)
				...
				{
					if (!leafCorrection)
						ptrmapPut(bt, ConvertEx::Get4(cell[i]), PTRMAP::BTREE, newPage->ID, &rc);
			...
			if (!leafCorrection)
			{
				for (i = 0; i < newPagesUsed; i++)
				{
					u3int2 key = ConvertEx::Get4(&newPages[i]->Data[8]);
					ptrmapPut(bt, key, PTRMAP::BTREE, newPages[i]->ID, &rc);
				}
			}
...
#pragma optimize("", on)
	...
	static int balance_deeper(MemPage *root, MemPage **childOut)
	{
		...
		if (rc == RC::OK)
		{
			rc = allocateBtreePage(bt, &child, &childID, root->ID, 0);
			copyNodeContent(root, child, &rc);
#if !OMIT_AUTOVACUUM
			if (bt.AutoVacuum)
				ptrmapPut(bt, childID, PTRMAP::BTREE, root->ID, &rc);
#endif
		}
		if (rc)
		{
		...
		_memcpy(child->OvflIdx, root->OvflIdxs, root->Overflows * sizeof(root->OvflIdx[0]));
	...
	static int balance(BtCursor *cur)
	{
...
#pragma region Insert / Delete / CreateTable / DropTable
	...
	RC Btree::Insert(BtCursor *cur, const void *key, int64 keyLength, const void *data, int dataLength, int zero, int appendBias, int seekResult)
	{
		if (cur->State == CURSOR::FAULT)
		{
			_assert(cur->SkipNext != RC::OK);
		...
		BtShared *bt = p->Bt;
		_assert(cursorHoldsMutex(cur));
		_assert(cur->WrFlag && bt->InTransaction == TRANS::WRITE && (bt->BtsFlags & BTS::READ_ONLY) == 0);
		...
		RC rc = saveAllCursors(bt, cur->IDRoot, cur);
		if (rc) return rc;
		...
		if (cur->KeyInfo == nullptr)
			invalidateIncrblobCursors(p, keyLength, 0);
		...
		TRACE("INSERT: table=%d nkey=%lld ndata=%d page=%d %s\n", cur->IDRoot, keyLength, dataLength, page->ID, loc == 0 ? "overwrite" : "new entry");
		_assert(page->IsInit);
...
end_insert:
	...
	RC Btree::Delete(BtCursor *cur)
	{
		...
		int cellIdx = cur->Idxs[cellDepth]; // Index of cell to delete
		...
		rc = saveAllCursors(bt, cur->IDRoot, cur);
		if (rc) return rc;
		...
		if (cur->KeyInfo == 0)
			invalidateIncrblobCursors(p, cur->Info.Key, 0);
	...
	static int btreeCreateTable(Btree *p, int *tableID, int createTabFlags)
	{
		BtShared *bt = p->Bt;
		_assert(HoldsMutex(p));
		_assert(bt->InTransaction == TRANS::WRITE);
		...
		if (bt->AutoVacuum)
		{
			...
			GetMeta(p, BTREE::LARGEST_ROOT_PAGE, &rootID);
			rootID++;
			...
			if (moveID != rootID)
			{
				...
				rc = btreeGetPage(bt, rootID, &root, 0);
				if (rc != RC::OK)
					return rc;
				...
				_assert(type != PTRMAP::ROOTPAGE);
				_assert(type != PTRMAP::FREEPAGE);
				rc = relocatePage(bt, root, type, ptrPageID, moveID, 0);
				releasePage(root);
			...
			ptrmapPut(bt, rootID, PTRMAP.ROOTPAGE, 0, &rc);
			if (rc)
			{
				releasePage(root);
				return rc;
		...
		_assert((bt->OpenFlags & BTREE_SINGLE) == 0 || rootID == 2);
		*tableID = (int)rootID;
		return RC.OK;
	}

	RC Btree::CreateTable(int *tableID, int flags)
	{
		Enter();
	...
	static RC clearDatabasePage(BtShared *bt, Pid id, bool freePageFlag, int *changes)
	{
		_assert(MutexEx::Held(bt->Mutex));
		if (id > btreePagecount(bt))
			return SysEx_CORRUPT_BKPT;
		...
		for (int i = 0; i < page->Cells; i++)
		{
			...
			if (!page->Leaf)
			{
				rc = clearDatabasePage(bt, ConvertEx::Get4(cell), 1, changes);
		...
		if (!page->Leaf)
		{
			rc = clearDatabasePage(bt, ConvertEx::Get4(&page->Data[8]), 1, changes);
			if (rc) goto cleardatabasepage_out;
		}
		else if (changes)
		{
...
cleardatabasepage_out:
	...
	RC Btree::ClearTable(int tableID, int *changes)
	{
		BtShared *bt = Bt;
		Enter();
		_assert(InTrans == TRANS::WRITE);
		...
		if (rc == RC.OK)
		{
			...
			rc = clearDatabasePage(bt, (Pid)tableID, 0, changes);
		}
		Leave();
		return rc;
	}

	static RC btreeDropTable(Btree *p, Pid tableID, int *movedID)
	{
		BtShared *bt = p->Bt;
		_assert(HoldsMutex(p));
		_assert(p->InTrans == TRANS::WRITE );
		...
		{
			sqlite3ConnectionBlocked(p->Ctx, bt->Cursor->Btree->Ctx);
			return RC::LOCKED_SHAREDCACHE;
		}
		...
		*movedID = 0;
		...
		if (tableID > 1)
		{
			...
			{
				Pid maxRootID;
				p->GetMeta(BTREE::LARGEST_ROOT_PAGE, &maxRootID);
	...
	RC Btree::DropTable(int tableID, int *movedID)
	{
		Enter();
...
#pragma region Meta
	...
	void Btree::GetMeta(int idx, uint32 *meta)
	{
		BtShared *bt = Bt;
		Enter();
	...
	RC Btree::UpdateMeta(int idx, uint32 meta)
	{
		...
		if (rc == RC::OK)
		{
			ConvertEx::Put4(&p1[36 + idx * 4], meta);
#ifndef OMIT_AUTOVACUUM
...
#pragma region Integrity Check
	...
	static void checkAppendMsg(IntegrityCk *check, char *msg1, const char *format, ...)
	{
		va_list ap;
...
#pragma region Settings
	...
	const char *Btree::GetFilename()
	{
		_assert(Bt->Pager != nullptr);
		return Bt->Pager->Filename(true);
	}

	const char *Btree::GetJournalname()
	{
		_assert(Bt->Pager != nullptr);
		return Bt->Pager->Journalname();
	}

	bool Btree::IsInTrans()
	{
		_assert(MutexEx::Held(Ctx->Mutex));
...
#ifndef OMIT_WAL
	RC Btree::Checkpoint(int mode, int *logs, int *checkpoints)
	{
		BtShared *bt = Bt;
		Enter();
		RC rc;
	...
	bool Btree::IsInReadTrans()
	{
		_assert(MutexEx::Held(Ctx->Mutex));
		return (InTrans != TRANS::NONE);
	}

	bool Btree::IsInBackup()
	{
		_assert(MutexEx::Held(Ctx->Mutex));
		return (Backups != 0);
	}

	void *Btree::Schema(int bytes, void (*free)(void *))
	{
		BtShared *bt = Bt;
		Enter();
		if (!bt->Schema && bytes)
		{
			bt->Schema = SysEx::TagAlloc(nullptr, bytes, true);
			bt->FreeSchema = free;
		}
		Leave();
		return bt->pSchema;
	...
	RC Btree::SchemaLocked()
	{
		_assert(MutexEx::Held(Ctx->Mutex));
		Enter();
		RC rc = querySharedCacheTableLock(this, MASTER_ROOT, READ::LOCK);
...
#ifndef OMIT_SHARED_CACHE
	RC Btree::LockTable(int tableID, bool isWriteLock)
	{
		_assert(InTrans != TRANS::NONE);
		RC rc = RC::OK;
		if (Sharable)
...
#ifndef OMIT_INCRBLOB
	RC Btree::PutData(BtCursor *cur, uint32 offset, uint32 amount, void *z)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(MutexEx::Held(Cur->Btree->Ctx->Mutex));
		...
		if (!cur->WrFlag)
			return RC::READONLY;
		_assert((cur->Bt->BtsFlags & BTS::READ_ONLY) == 0 && cur->Bt->InTransaction == TRANS::WRITE);
		_assert(hasSharedCacheTableLock(cur->Btree, cur->IDRoot, 0, 2));
		_assert(!hasReadConflicts(cur->Btree, cur->IDRoot));
		_assert(cur->Pages[cur->Page]->IntKey);

		return accessPayload(cur, offset, amount, (unsigned char *)z, 1);
	}

	void Btree::CacheOverflow(BtCursor *cur)
	{
		_assert(cursorHoldsMutex(cur));
		_assert(MutexEx::Held(cur->Btree->Ctx->Mutex));
		invalidateOverflowCache(cur);
	...
	RC Btree::SetVersion(int version)
	{
	...
	void Btree::CursorHints(BtCursor *cur, unsigned int mask)
	{
