...
namespace Core.IO
{
    public class CoreVFile : VFile
    {
        ...
        public override RC Close()
        {
            ...
            return (rc ? RC.OK : SysEx.OSError(RC.IOERR_CLOSE, "winClose", Path));
        }
        ...
        public override RC Read(byte[] buffer, int amount, long offset)
        {
            ...
            catch (Exception) { LastErrorID = (uint)Marshal.GetLastWin32Error(); return SysEx.OSError(RC.IOERR_READ, "winRead", Path); }
            if (read < amount)
            {
        ...
        public override RC Write(byte[] buffer, int amount, long offset)
        {
            ...
            if (rc == 0 || amount > (int)wrote)
            {
                ...
                return (LastErrorID == CoreVFileSystem.ERROR_HANDLE_DISK_FULL || LastErrorID == CoreVFileSystem.ERROR_DISK_FULL ? RC.FULL : SysEx.OSError(RC.IOERR_WRITE, "winWrite", Path));
            }
            return RC.OK;
        }
        ...
        public override RC Truncate(long size)
        {
            ...
            catch (IOException) { LastErrorID = (uint)Marshal.GetLastWin32Error(); rc = SysEx.OSError(RC.IOERR_TRUNCATE, "winTruncate2", Path); }
        ...
        public override RC Unlock(LOCK lockType)
        {
            ...
            if (type >= LOCK.EXCLUSIVE)
            {
                ...
                if (lockType == LOCK.SHARED && getReadLock() == 0)
                    ...
                    rc = SysEx.OSError(RC.IOERR_UNLOCK, "winUnlock", Path);
            }
        ...
        private int seekWinFile(long iOffset)
        {
            ...
            catch (Exception) { LastErrorID = (uint)Marshal.GetLastWin32Error(); SysEx.OSError(RC.IOERR_SEEK, "seekWinFile", Path); return 1; }
            return 0;
        }
        ...
        private int unlockReadLock()
        {
            ...
            if (res == 0) { LastErrorID = (uint)Marshal.GetLastWin32Error(); SysEx.OSError(RC.IOERR_UNLOCK, "unlockReadLock", Path); }
            return res;
        }

        #region Locking

        private static LockingStrategy _lockingStrategy = (CoreVFileSystem.IsRunningMediumTrust() ? new MediumTrustLockingStrategy() : new LockingStrategy());
